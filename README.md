...
操作系统的开发始终涉及诸多艰难因素，现实情境呈现出操作系统（例如Linux）在漫长演进中逐渐演变为庞大且复杂的趋势，导致其维护和开发变得困难重重。未来的操作系统或许不再像当前主宰全球的Linux和Windows一样庞大且通用，而可能会转向多样化的组件化定制操作系统，这些操作系统能够迅速组合各类组件，呈现丰富多样的功能，同时能够高效适应未来多样化的处理器、加速器、外设以及应用需求。从开发便利性、性能表现到安全性等方面，这种新型操作系统都有望超越现有的通用操作系统。然而，如何设计这种组件化定制操作系统则涉及一个需要深思熟虑的具有挑战性的议题。基于此，ArceOS的开发团队一直在思考一个关键问题，即是否可以通过构建基于泛型的独立功能组件，来实现快速构建针对不同领域的专用操作系统。

操作系统开发面临的难题在于，开发人员需要进行繁琐的操作系统软件编写，需关注的技术细节极多。这一现象的根源在于操作系统内部模块之间错综复杂的依赖关系，所引发的软件复杂性，以及操作系统内在的自包含特性，从而导致软件的难以重用性。因此，将操作系统视作多层次的软件堆栈，并进行精细的功能层次划分，使得各功能模块具有独立性，这将有助于构建基于操作系统层次的分层软件开发策略。
...
众所周知，输出都是通过硬件完成的，正如你现在正在看的这份文档，它能显示在屏幕上都得益于我们的操作系统为我们封装好了硬件操作并能够执行它。而helloworld unikernel没有这层操作系统的支持，必须添加上与硬件相关的层的封装，axhal 就应运而生了。
...
在ArceOS中，axhal组件为不同硬件平台提供了一层硬件封装。该组件负责特定操作平台的引导和初始化过程，并提供对硬件的操作。例如，在路径“modules/axhal/src/platform/qemu_virt_loongarch64/console.rs”中，提供了字符输出的封装。程序可以直接调用其中的putchar函数进行字符输出，而不需要重复使用汇编级别的代码，如sbi指令。

实际上，在ArceOS中，经过多层封装的 putchar 函数已经被转换为类似于print函数的便于用户使用的形式。然而，helloworld 程序的输出本质上仍然是通过调用 axhal 组件的putchar函数实现的。接下来的内容将详细介绍如何移除为用户提供的便捷封装，通过对axhal代码进行适当修改，直观地体验系统启动并调用 axhal 提供的putchar函数进行输出的流程。
...
至此，我们已经成功完成了从系统引导到输出的最简流程。值得注意的是，这一过程在从开机到输出的阶段，并不依赖于axhal组件以外的任何其他模块。这一点充分体现了ArceOS的设计理念，即只需重复使用此单一模块，便能轻松地进行硬件操作。

**2. helloworld 程序基于 axhal 组件实现输出**

基于先前所述， axhal 组件已经准备好了 putchar 输出函数，接下来可以编写 helloworld 程序进行输出了，如果关注了后续内容中 helloworld 运行的完整流程图就可以发现 helloworld 的运行还依赖于 libax，libax 就是提供一层类似于库函数的封装，它实现输出的本质还是用到 axhal 中的输出，在此部分更多考虑地是 helloworld 与 axhal 组件进行搭配并且输出的过程，libax 可以看成是 helloworld 程序的一部分。
...
对于动态分配的类型，比如创建一个 String 类型的字符串，然后通过 println 的格式化功能将组装后的字符串打印到屏幕。现在我们来讨论如何实现这一功能。

进一步分析上述的需求，其目的其实是操作系统 ArceOS 为用户的应用程序 helloworld 提供内存分配支持，那么如何优雅地为用户提供这种支持成了我们目前首要的目标。

结合前文对Unikernel概念的介绍，我们知道虽然操作系统种类繁多，但通过深度拆分和罗列各个功能，可以发现支撑操作系统的功能模块具有共性。例如，要实现多任务切换的操作系统必须涉及任务（进程）调度算法，这种可分离为独立模块，甚至可以被其他操作系统复用。在ArceOS的设计体系中，这种特点被称为系统无关性。相反地，之前提到的axhal和axruntime是ArceOS自身利用系统无关的组件为用户程序创建特定运行时环境的方式。这种组合扩展方式与其他操作系统不同。因此，这类模块被称为系统相关，代表了操作系统通过系统无关组件来构建用户程序运行环境的独特方式，无法被其他系统复用。
...
ArceOS在用户视野之外，根据用户选定的特性需求，以一种不可见的方式，组装和扩展各种模块，从而提供定制的运行环境。这实际上揭示了ArceOS的核心机制，即组件化运作。本节内容从最简单的 helloworld 使用动态内存分配特性出发，“窥一斑而知全豹”，来触及 ArceOS 的核心运作机制。在后续的章节中将继续介绍 ArceOS 更为强大的功能特性。
...
LoongArch的寄存器包括通用寄存器（GR）、浮点寄存器（FR）、向量寄存器（VR） 和用于特权模式（PLV0）的控制状态寄存器（CSR）。
...
Opcode是指令操作码，Rj和Rk是源操作数（寄存器），Rd是目标操作数（寄存器），Ra是 4R-type格式特有的附加操作数（寄存器）。I8/I12/I14/I16/I21/I26分别是8位/12位/14位/ 16位/21位/26位的立即数。其中较长的21位和26位立即数在指令字中被分割为高位部分与低位 部分，所以在这里的格式描述中能够看到I21L/I21H和I26L/I26H这样带后缀的表述。
...
3. 位域操作指令:
   ...
   REVB.2H REVB.4H REVB.2W REVB.D REVH.2W REVH.D BITREV.W BITREV.D
   MASKEQZ MASKNEZ
   ```
...
8. 特殊指令:
   ...
   ```
   SYSCALL BREAK CPUCFG RDTIMEL.W RDTIMEH.W
   ASRTLE.D ASRTGT.D
   ```
...
LoongArch可以使用直接映射地址翻译模式和页表映射地址翻译模式。

直接映射地址翻译模式通过CSR.DMWn（n=0~3）寄存器来分别设置四个直接映射配置窗口。每个窗口除了地址范围信息外，还可以配置该窗口在哪些特权等级下可用，以及虚地址落在该窗口上的访存操作的存储访问类型。
...
所有特权指令仅在PLV0 特权等级下才能访问。仅有一个例外情况 ， 当 CSR.MISC中 的 RPCNTL1/RPCNTL2/RPCNTL3 配置为 1 时，可以在 PLV1/PLV2/PLV3 特权等级下执行CSRRD 指令读取性能计数器。在本实验中并不涉及到性能计数器，因此可以不用关注。在实验中，现阶段常用到的特权指令有:
...
LoongArch包括32个通用寄存器（ `$r0` ~ `$r31` ），LA32中每个寄存器为32位宽， LA64中每个寄存器为64位宽。 `$r0` 的内容总是固定为0，而其他寄存器在体系结构层面没有特殊功能。（ `$r1` 算是一个例外，在BL指令中固定用作链接返回寄存器。）
